...
- **Scheduling**: schedule_task (at/every/cron), list_tasks, complete_task, cancel_task, pause_task, resume_task, update_task_checkpoint.
...
- **Heartbeat**: hourly isolated session checks HEARTBEAT.md; respond `HEARTBEAT_OK` if nothing needs attention.
...
- **Chat history**: list_chat_logs, read_chat_log (daily logs).
