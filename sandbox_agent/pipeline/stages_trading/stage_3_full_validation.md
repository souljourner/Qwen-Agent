...
- `oos_sortino / pilot_sortino >= 0.5` — the single best honesty check (annualized Sortino is the KEY metric for comparing strategies; compute both segments via `compute_metrics(led)`, which returns `sortino` and `annualized_return_pct` directly).
- `walk_forward_win_rate >= 0.6` — strategy beats benchmark (risk-adjusted) in ≥ 60% of rolling windows.
- `total_trades >= 20` over the full period (bare statistical floor — do NOT engineer extra trades to hit a frequency target).
- `pilot_annualized_return_pct` and `oos_annualized_return_pct` present (report-only — headline numbers for the verdict and completion email; their values are never gated).
